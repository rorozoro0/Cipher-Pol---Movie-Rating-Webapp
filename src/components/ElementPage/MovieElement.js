...
function ElementDisplay(props){
  ...
  function rateNow(e){
    e.preventDefault();
      ...
      Axios.patch(`https://cors-anywhere.herokuapp.com/https://rorolist1.herokuapp.com/api/v1/movies/${randomdata._id}`,{
        rating: {
          avgrating :  avg,
          nviews: views
        }
  ...
  const onLikesClick = () => {
    ...
    Axios.patch(`https://cors-anywhere.herokuapp.com/https://rorolist1.herokuapp.com/api/v1/movies/${randomdata._id}`, {likes: randomdata.likes+1}).then((response) => {
  ...
  useEffect(() => {
    Axios.get(`https://cors-anywhere.herokuapp.com/https://rorolist1.herokuapp.com/api/v1/movies/${props.match.params.id}`).then((response) => {
          setrandom(response.data.data.movie)
          document.title = response.data.data.movie.name + " (Movie) - Cipher-Pol"
    })
  },[])
        ...
        <StyledContainer theme={theme} >
              ...
              <Title>{randomdata.name+` ‧ `+randomdata.category}</Title>
                  ...
                  Rating: {randomdata.rating.avgrating+'/10'}<FaStar style={{color:'orange'}}/><span style ={{color: "#ccc", fontWeight: 100}}>{`(based on ${randomdata.rating.nviews} reviews)`}&nbsp;</span>
                    ...
                    <Date1>{`Director : ${randomdata.director}`}</Date1>
                    
                    <Description>{randomdata.plot}</Description>
              </Flexing>
        </StyledContainer>
      </StyledContainer1>
      <CommentSection id = {props.match.params.id} data = "movie" category = "movies" details = {{rating: randomdata.rating, comments: randomdata.comments}}></CommentSection>
    </StyledRoot>
  )
}
