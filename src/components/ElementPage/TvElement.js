...
function ElementDisplay(props){
  ...
  function rateNow(e){
    e.preventDefault();
      ...
      Axios.patch(`https://cors-anywhere.herokuapp.com/https://rorolist1.herokuapp.com/api/v1/tv/${randomdata._id}`,{
        rating: {
          avgrating :  avg,
          nviews: views
        }
  ...
  const onLikesClick = () => {
    ...
    Axios.patch(`https://cors-anywhere.herokuapp.com/https://rorolist1.herokuapp.com/api/v1/tv/${randomdata._id}`, {likes: randomdata.likes+1}).then((response) => {
  ...
  useEffect(() => {
    Axios.get(`https://cors-anywhere.herokuapp.com/https://rorolist1.herokuapp.com/api/v1/tv/${props.match.params.id}`).then((response) => {
          setrandom(response.data.data.tv)
          document.title = response.data.data.tv.name + " (Tv) - Cipher-Pol"
    })
  },[])
  ...
  return (
    <StyledRoot>
        ...
        <StyledContainer theme={theme} >
              ...
              <Title>{randomdata.name+` ‧ `+randomdata.category}</Title>
                  ...
                  Rating: {randomdata.rating.avgrating+'/10'}<FaStar style={{color:'orange'}}/><span style ={{color: "#ccc", fontWeight: 100}}>{`(based on ${randomdata.rating.nviews} reviews)`}&nbsp;</span>
                    ...
                    <Date1> {`Seasons : ${randomdata.season}`}</Date1>
                    <Date1>{`Number of Episodes : ${randomdata.nepisode}`}</Date1>
      ...
      <CommentSection id = {props.match.params.id} category = "tv" data = "tv" details = {{rating: randomdata.rating, comments: randomdata.comments}}></CommentSection>
    </StyledRoot>
  )
}
